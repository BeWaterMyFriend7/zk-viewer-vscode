...
- **中英文界面**：导入 / 导出按钮、选择框、进度和结果提示跟随 VS Code 显示语言自动切换中文或英文
...
4. 右键节点执行新增、删除、复制路径和导出；侧边栏 `ZooKeeper Nodes` 标题栏提供导入按钮；双击打开详情面板查看 JSON（只读），点击 **Edit** 进入编辑
...
- 右键节点可执行 **新增节点 / 编辑节点数据 / 删除节点 / 复制路径 / 导出节点数据 / 导出完整子树 / 导入节点数据 / 刷新**
...
- 连接后点击 `ZooKeeper Nodes` 标题栏的 **导入图标**，或右键选择 `Import Node Data...`，可读取上述 JSON 并按完整路径恢复节点
- 导入前选择已存在节点的处理方式：**跳过**会保留已有数据，**覆盖**会使用版本校验更新数据；缺失节点按父级优先创建为持久节点
- 文件格式、路径范围、重复路径、Base64 数据和外部父节点会在写入前校验，尽量避免损坏文件造成部分导入
- 相关按钮和交互提示根据 VS Code 显示语言自动使用中文或英文
