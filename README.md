...
- 右键节点可执行 **新增节点 / 编辑节点数据 / 删除节点 / 复制路径 / 刷新**
...
搜索结果按路径排序展示，点击结果后树视图自动展开并定位到对应节点。「精确路径」模式直接输入完整路径（如 `/app/config`）即可直达并选中该节点，不会做模糊匹配。内容搜索自动跳过空数据与超大节点，超大节点阈值由 `zkViewer.maxNodeDataBytes` 控制。搜索默认最多遍历 50000 个节点（`zkViewer.maxSearchNodes`）；若达到上限，界面会提示结果可能不完整，可提高上限或限定子树范围后重试。
...
3. 数据为 JSON 时自动格式化展示；非 JSON 文本原样展示；二进制数据以十六进制视图只读展示
4. 数据默认**只读**，点击 **Edit** 按钮才进入编辑模式，避免误触修改
5. 点击 **Save** 保存：写入携带节点版本号（乐观锁），若期间被其他端修改则提示版本冲突且**不会覆盖**，刷新后重新编辑即可
