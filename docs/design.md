...
- `ConnectionStore`：连接配置持久化到 `globalState`，同一 Profile、同一扩展运行环境下跨窗口与工作区共享；初始化时迁移当前工作区旧 `workspaceState` 中的配置，先按 ID、再按规范化的 `hosts + username` 判重（账号未填写、空串和纯空格等价），命中已有全局连接时直接丢弃旧项且不覆盖，并以工作区级版本标记保证迁移只成功执行一次。普通用户新增不做该业务判重。密码单独存 `SecretStorage`，配置中永不出现明文；
...
- `node-model`：由 `ephemeralOwner`（将 8 字节零值规整为 `0x0`，可用 `isZeroId` 判断）与顺序命名（`-\d{10}$`）推导四种节点类型；`iconForType(type, isLeaf)` 为普通持久节点按是否有子节点映射文件夹 / 文件图标，持久顺序、临时、临时顺序保留专属 Codicon；`iconColorForType(type, isLeaf)` 返回扩展自定义色键（文件夹 / 文件、顺序 / 临时节点各有归属色），在 `NodeTreeProvider` 中通过 `new ThemeIcon(id, new ThemeColor(key))` 给图标着色。
