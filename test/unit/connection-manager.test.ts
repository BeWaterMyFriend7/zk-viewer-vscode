...
describe('ConnectionManager', () => {
  ...
  it('reconnects after connection loss and stops after the maximum', async () => {
    ...
    assert.strictEqual(manager.getState(), 'connected');
    assert.strictEqual(clients.length, 2, 'a fresh client should be created for the retry');
    assert.strictEqual(clients[0].closeCalls, 1, 'the lost client should be closed');
  });
